const jp = {
  ...
  reactivateAccount: {
    title: "アカウントを再有効化",
    description: "お客様のアカウントは1年以上ログインがなかったため、アーカイブされました。本人確認のため現在のパスワードを入力し、新しいパスワードを設定して再有効化してください。",
    currentPasswordLabel: "現在のパスワード",
    currentPasswordPlaceholder: "以前のパスワードを入力",
    currentPasswordHelp: "アカウントがアーカイブされる前に使用していたパスワードを入力してください",
    ...
    errors: {
      ...
      invalidCredentials: "パスワードが無効です。確認してもう一度お試しください。",
    },
  },
  ...
  dataRequestTitle: "d) データのリクエストと削除",
  dataRequestDescription:
    "このプライバシーポリシーに基づき、次の権利を有します：",
  dataRequestList: [
    "あなたの個人データへのアクセスを要求する。",
    "私たちが保持するあなたの個人データの修正を要求する。",
    "あなたの個人データの削除を要求する。",
    "あなたの個人データの転送を要求する。",
    "同意を撤回する。",
  ],
  surveysTitle: "e) アンケート、抽選、プロモーション",
  surveysDescription:
  ...
  internetActivityTitle: "f) インターネット活動情報",
  ...
  cookiesTableRows: [
    ...
    },
    {
      type: "パフォーマンスクッキー",
      description:
        "これらのクッキーは、サイトの使用方法に関する情報を収集します。頻繁にアクセスされたページや、特定のページでエラーメッセージが表示されたかどうかなどです。これらのクッキーは、あなたを個別に識別する情報を収集することはありません。情報はサイトの機能とパフォーマンスを改善するためにのみ使用されます。",
      managingSettings:
    ...
    },
    {
      type: "機能性クッキー",
      description:
        "機能性クッキーは、入力した情報や選択したオプション（ユーザー名、言語、地域など）を記憶し、よりパーソナライズされた機能を提供するために使用されます。",
      managingSettings:
        "機能性クッキーを無効にする方法については、ブラウザの設定で確認してください。Flashクッキーのプライバシーとストレージ設定を管理する方法についてはこちらをご覧ください。",
    },
  ],

  securityTitle: "XI. セキュリティ",
  securityDescription:
    "私たちは、個人情報の損失、不正使用、不正アクセス、開示、変更、または破壊から保護するために、合理的かつ適切な措置を講じています。これらの措置は、処理に関連するリスクや個人情報の性質を考慮しています。しかし、これらの努力にもかかわらず、個人情報が送信中または私たちのシステム上で保存中に完全に保護されることを保証することはできません。",

  changesTitle: "XII. この通知の変更",
  changesDescription:
  ...
  donateButton: "寄付する",
  DonorAboutTitle: "ジェイソン・ダシルヴァについて知ろう",
