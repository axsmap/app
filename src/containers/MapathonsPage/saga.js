...
import {
  getInactiveMapathonsEndpoint,
  getMapathonsEndpoint
} from '../../api/mapathons'
...
function* getMapathonsFlow() {
  ...
  const getMapathonsParams = {
    ...
    hideInactiveMapathons:
      filters.hideInactiveMapathons === 1
        ? `${filters.hideInactiveMapathons}`
        : undefined
  }
...
function* getMapathonsInactiveFlow() {
  const sendingRequest = yield select(makeSelectApp('sendingRequest'))
  if (sendingRequest) {
    return
  }
  ...
  let response
  try {
    response = yield call(getInactiveMapathonsEndpoint, getMapathonsParams)
  } catch (error) {
    yield put(setNotificationType('error'))
...
export function* mapathonsInactiveSaga() {
  yield takeLatest(GET_MAPATHONS, getMapathonsInactiveFlow)
}
