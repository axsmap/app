...
class Form extends Component {
  ...
  setSelectedPlace = (place) => {
    this.setState({
      data: { ...this.state.data, address: place.name },
    })
    this.props.setStartingPoint(place.location)
  ...
  fetchSuggestions = (event) => {
    ...
    const newTimeoutId = setTimeout(() => {
      if (value.length >= 3) {
        placeAutocompleteEndpoint({ input: value }).then((response) => {
  ...
  onLocationChange = (event) => {
    this.props.setLocationCoordinates(event)

    const coordinate = `${event.latLng.lat()},${event.latLng.lng()}`

    if (coordinate) {
      reverseGeocodeEndpoint({ coordinate }).then((response) => {
    ...
    return (
      <Wrapper>
        ...
        <Map
          ...
          onLocationChange={(event) => this.onLocationChange(event)}
        />
